...
int numberInput() 
{
    bool readlineFromStrToInt = true; //Метод ввода и проверки на число
    int number =0;
    while (readlineFromStrToInt){
        ...
        {
            number = numberInt;  //проверка на число
            if (number < -9 )
            {
...
int symbolSumm(int number)
{
    int sum =0;  //Модуль суммы цифр в числе через символы
...
{
    int ost =0;  //Модуль математический
    int sum =0;
    while ( number > 9)
    {
        ost = number%10;
